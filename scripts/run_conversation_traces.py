...
def run_conversation_trace(chain: RAGChain, conv_data: dict, trace_file: Path) -> dict:
    ...
    with open(trace_file, "w", encoding="utf-8") as f:
        ...
        for turn_idx, turn in enumerate(turns, 1):
            ...
            if is_safe:
                ...
                rag_start = time.time()
                try:
                    rag_result = chain.query_with_metadata(query, session_id=trace_session_id, language=language)
...
def main():
    ...
    for i, conv in enumerate(conversations, 1):
        ...
        try:
            ...
            results.append(
                {"session_id": session_id, "status": "ERROR", "error": str(e), "issues": [f"[CRITICAL] Exception: {e}"]}
            )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for r in results:
            ...
            f.write(
                f"| {r['session_id']} | {desc} | {r.get('num_turns', 0)} | {r['status']} | {len(r.get('issues', []))} |\n"
            )
        f.write("\n")
    ...
    summary = {
        ...
        "results": {"passed": passed, "failed": failed, "warnings": warned, "errors": errors},

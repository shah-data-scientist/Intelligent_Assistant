...
class ServiceManager:
    ...
    def rebuild_index(self):
        ...
        try:
            subprocess.run(["poetry", "run", "python", str(rebuild_script)], cwd=self.project_root, check=True)
    ...
    def start_api(self):
        ...
        self.api_process = subprocess.Popen(
            cmd, cwd=self.project_root, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1
        )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Start Cultural Events RAG Assistant services")
    ...
    parser.add_argument("--rebuild", action="store_true", help="Rebuild FAISS index before starting")

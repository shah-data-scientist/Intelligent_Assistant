...
def evaluate_single_query(query_data: dict, chain: RAGChain, storage: EventStorage, session_id: str = None) -> dict:
    ...
    should_ask_clarification = expected_behavior.get("should_ask_clarification", False) or generation_expectations.get(
        "should_ask_clarification", False
    )

    # Run query through chatbot
    try:
        result = chain.query_with_metadata(query_text, session_id=session_id, language=language)
    ...
    if not chatbot_success:
        issues.append({"type": "ERROR", "message": f"Chatbot failed: {error}"})
    ...
    # Special query type validation
        ...
        if query_type == "greeting":
            ...
            if not any(ind in answer_lower for ind in indicators):
                issues.append({"type": "MISSING_GREETING_RESPONSE", "message": "Expected greeting response not found"})
        ...
        # For off-topic queries: check for polite redirect
            ...
            if not any(ind in answer_lower for ind in redirect_indicators):
                issues.append({"type": "MISSING_REDIRECT", "message": "Expected polite redirect not found in response"})
        ...
        # For meta queries: check for capability explanation
            ...
            if not any(ind in answer_lower for ind in capability_indicators):
                issues.append(
                    {"type": "MISSING_CAPABILITY_EXPLANATION", "message": "Expected capability explanation not found"}
                )
    ...
    for keyword in must_contain:
        if not keyword_matches(keyword, answer_lower):
            warnings.append(
                {"type": "MISSING_KEYWORD", "message": f'Expected keyword "{keyword}" not found in response'}
            )

    for keyword in must_not_contain:
        if keyword.lower() in answer_lower:
            issues.append({"type": "FORBIDDEN_KEYWORD", "message": f'Forbidden keyword "{keyword}" found in response'})
...
def main():
    ...
    parser.add_argument("--input", default="data/evaluation/golden_dataset.json", help="Golden dataset JSON file")
    ...
    parser.add_argument("--conversations-only", action="store_true", help="Only evaluate conversations")
    parser.add_argument("--singles-only", action="store_true", help="Only evaluate single queries")
    ...
    if not args.singles_only and conversations:
        ...
        for i, conv in enumerate(conv_to_eval, 1):
            ...
            status_emoji = (
                "[PASS]" if result["status"] == "PASS" else ("[WARN]" if result["status"] == "WARN" else "[FAIL]")
            )

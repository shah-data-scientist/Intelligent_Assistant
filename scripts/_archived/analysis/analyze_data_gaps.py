...
has_price = sum(
    ...
    if e.conditions
    and (
        "gratuit" in e.conditions.lower()
        or "free" in e.conditions.lower()
        or "tarif" in e.conditions.lower()
        or "price" in e.conditions.lower()
    )
)
...
has_age_info = sum(
    ...
    for e in all_events
    if e.description
    and any(age in e.description.lower() for age in ["enfant", "children", "ans", "years old", "âge", "age"])
)
...
free_events = [
    e for e in all_events if e.conditions and ("gratuit" in e.conditions.lower() or "free" in e.conditions.lower())
]
print("\n" + "-" * 80)
...
print("-" * 80)
print(
    f"Events with accessibility info: {len(accessible_events)}/{len(all_events)} ({len(accessible_events)/len(all_events)*100:.1f}%)"
)
...
multilingual_events = [
    ...
    if e.description
    and any(
        lang in e.description.lower() for lang in ["english", "anglais", "spanish", "espagnol", "german", "allemand"]
    )
]
print("\n" + "-" * 80)
...
print("-" * 80)
print(
    f"Events mentioning other languages: {len(multilingual_events)}/{len(all_events)} ({len(multilingual_events)/len(all_events)*100:.1f}%)"
)

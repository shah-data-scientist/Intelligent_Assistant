...
classical_keywords = [
    ...
    "opéra",
    "opera",  # Opera counts as classical
]
...
# Count matches
with_filter_matches = sum(
    1
    for event, _ in results
    if any(
        kw in f"{event.title} {event.description or ''} {event.scraped_content or ''}".lower()
        for kw in classical_keywords
    )
)
without_filter_matches = sum(
    1
    for event, _ in results_no_filter
    if any(
        kw in f"{event.title} {event.description or ''} {event.scraped_content or ''}".lower()
        for kw in classical_keywords
    )

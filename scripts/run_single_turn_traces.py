...
def run_single_trace(
    ...
    with open(trace_file, 'w', encoding='utf-8') as f:
        ...
        known_cities = ["paris", "versailles", "montreuil", "nanterre",
                       "saint-denis", "bobigny", "creteil", "bondy",
                       "fontainebleau", "vincennes"]
        ...
        try:
            ...
            if scope_dim and scope_dim.detected:
                ...
                if scope_dim.value == "all":
                    output(f"    - Broad Intent Bypass: YES (user explicitly asked for 'all')")
        ...
        if should_run_rag:
            ...
            try:
                ...
                result['issues'].append({
                    'severity': 'HIGH',
                    'issue': f'RAG pipeline error: {str(e)}'
                })
        else:
            output("")
            output("-" * 60)
...
def generate_analysis_report(results: list[dict], report_path: Path):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\n---\n")
        f.write(f"Report generated by `run_single_turn_traces.py`\n")
...
def main():
    ...
    if not api_ok:
        ...
        print(f"  - Check your .env file has GOOGLE_API_KEY set")
        print(f"  - Check no Windows env var is overriding it")
        print(f"  - Wait a minute if rate limited")
        print("=" * 80)
        return
        ...
        if has_api_error:
            ...
            if error_type == 'RATE_LIMIT':
                if consecutive_api_errors >= MAX_RETRIES_ON_RATE_LIMIT:
                    ...
                    print(f"  Stopping to avoid wasting time. Please wait and retry later.")
                    break
                else:
                ...
                print(f"  Please check your GOOGLE_API_KEY environment variable.")
                break
        else:

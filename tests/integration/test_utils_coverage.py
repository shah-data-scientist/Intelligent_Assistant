"""
FILE: test_utils_coverage.py
STATUS: Active
RESPONSIBILITY: Integration tests for utility module coverage.
...
def mock_db_data():
    ...
    cursor.execute(
        'INSERT INTO events VALUES (\'Poissy\', NULL, \'{"location_coordinates": {"lat": 48.9298, "lon": 2.0441}}\')'
    )
    ...
    # Date Keywords
    cursor.execute(
        "INSERT INTO search_keywords VALUES ('janvier', 'date', 'fr', 'january', NULL, '[\"janv\", \"janveir\"]')"
    )
    cursor.execute("INSERT INTO search_keywords VALUES ('this weekend', 'date', 'en', 'weekend', NULL, '[]')")

    # Event Keywords
    cursor.execute(
        "INSERT INTO search_keywords VALUES ('concert', 'event', 'fr', 'concert', 'Musique', '[\"concer\"]')"
    )

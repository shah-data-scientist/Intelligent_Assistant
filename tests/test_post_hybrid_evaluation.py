"""Evaluate system performance after hybrid search implementation."""
...
def main():
    try:
        ...
        report = evaluator.run_full_evaluation(
            ...
            retrieval_k=5,
            session_id=f"post_hybrid_eval_{uuid.uuid4().hex[:8]}"
        )
        ...
        for complexity, stats in sorted(complexity_stats.items()):
            ...
            logger.info(f"  {complexity.upper()}: Quality={avg_quality:.3f}, Faith={avg_faith:.3f}, Rel={avg_rel:.3f} ({stats['count']} queries)")

        # Genre-specific breakdown
        logger.info("\nGenre-Specific Query Performance:")
        genre_queries = [
            r for r in report.per_query_results
            if any(kw in r.get("query", "").lower() for kw in ["jazz", "classique", "classical", "rock", "électronique"])
        ]
        if genre_queries:
            ...
            logger.info(f"  Genre queries ({len(genre_queries)}): Quality={avg_genre_quality:.3f}, Faith={avg_genre_faith:.3f}, Rel={avg_genre_rel:.3f}")

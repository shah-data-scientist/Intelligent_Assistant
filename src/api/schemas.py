...
class StructuredEvent(BaseModel):
    ...
    title: str
    date: str  # Date only (YYYY-MM-DD format), NOT including time
    city: str
    location: str = "Unknown"  # Always required, show "Unknown" if not available
    url: Optional[str] = None
    price_label: str = "Unknown"  # Always required
    age_label: str = "Unknown"  # Always required
    times: List[str] = Field(default_factory=list, description="List of available times if event has multiple showings")
    times_display: str = "Unknown"  # Always required, show "Unknown" if not available
...
class ChatResponse(BaseModel):
    ...
    needs_clarification: bool = Field(
        default=False, description="True if the query was too broad and clarifying questions were asked"
    )
    clarifying_questions: List[str] = Field(
        default_factory=list, description="List of clarifying questions asked to the user"
    )

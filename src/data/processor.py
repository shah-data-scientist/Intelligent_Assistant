...
class EventProcessor:
    ...
    JUNK_PHRASES = [
        ...
        r"inscription obligatoire",
        r"partager cet événement",
        r"cliquez ici",
        r"powered by openagenda",
        r"cet événement est proposé par",
        r"les acceptez-vous",
        r"accepter",
        r"refuser",
        r"matomo",
        r"tous les événements",
        r"partager / exporter",
        r"outils d'inscription",
        r"s'inscrire / réserver",
        r"information additionnelle",
        r"aucune saisie",
        r"suggérer une modification",
        r"Catalogues départementaux",
        r"Catalogue national",
        r"structures d’accueil",
        r"hébergement",
        r"Plan du site",
        r"Mentions légales",
    ]
    ...
    def classify_category(self, event: Event) -> str:
        ...
        search_text = (
            f"{event.title} {event.description or ''} {event.scraped_content or ''} {' '.join(event.tags)}".lower()
        )
    ...
    def process_record(self, record: dict[str, Any]) -> list[Event]:
        ...
        try:
            ...
            api_desc = self.safe_normalize(
                fields.get("longdescription_fr") or fields.get("description_fr") or fields.get("description")
            )
            ...
            if acc and "voir site" in acc.lower():
                acc = ""
            ...
            if not parsed_timings:
                ...
                end_date = self.parse_date(
                    fields.get("lastdate_end") or fields.get("firstdate_end") or fields.get("end_date")
                )
                parsed_timings = [
                    {
                        "begin": start_date.isoformat() if start_date else None,
    ...
    def deduplicate_events(self, events: list[Event]) -> list[Event]:
        ...
        logger.info(
            f"Deduplication: {len(events)} -> {len(unique_events)} events (merged {len(events) - len(unique_events)} same-day duplicates)"
        )
    ...
    def filter_paris_events(self, events: list[Event]) -> list[Event]:
        ...
        return [e for e in events if e.location and e.location.city and "paris" in e.location.city.lower()]

    def filter_by_date_range(self, events: list[Event], start_date: datetime, end_date: datetime) -> list[Event]:
        """Filter events within a specific date range."""
        return [e for e in events if e.start_date and start_date <= e.start_date <= end_date]
    ...
    def filter_ile_de_france_events(self, events: list[Event]) -> list[Event]:
        ...
        return [
            e
            for e in events
            if e.location and e.location.postal_code and e.location.postal_code[:2] in idf_postal_prefixes
        ]
